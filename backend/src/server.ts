...
interface ApiFootballFixture { fixture?: { id?: number; date?: string; venue?: { name?: string }; status?: { short?: string; long?: string; elapsed?: number | null } }; league?: { name?: string }; teams?: { home?: { name?: string }; away?: { name?: string } }; goals?: { home?: number | null; away?: number | null } }
...
class ApiFootballProvider implements VanguardProvider {
  ...
  private async fetchMatches(date: string) {
    ...
    return (payload.response || []).map(mapFixture).filter((x): x is MatchData => Boolean(x));
  }
  async getMatches() {
    ...
    try {
      const data = (await this.fetchMatches(date)).sort((a, b) => {
      ...
      const value = { data, dataMode: 'API_FOOTBALL', live: data.some(m => m.dataSource === 'LIVE_PROVIDER'), fetchedAt: new Date().toISOString(), date };
      this.cache = { date, fetchedAtMs: now, value }; return value;
    } catch (error) {
...
export class MockProvider implements VanguardProvider { async getMatches() { return { data: [], dataMode: 'DEVELOPMENT_MOCK', live: false, fetchedAt: new Date().toISOString(), date: localDate(env.SPORTS_TIMEZONE) }; } async queryAI(prompt: string) { return `Development-only AI response for: "${prompt}".`; } }
